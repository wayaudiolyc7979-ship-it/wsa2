...
class _SettingsBtn(QPushButton):
    """타이틀바용 설정 버튼 — 누르면 설정창 오픈. Lucide 'settings'(기어) 라인 아이콘으로
    앱 전체 아이콘과 통일(2026-08-30 SPL Alarm 리디자인)."""
    def __init__(self):
        super().__init__()
    ...
    def paintEvent(self, e):
        ...
        pm = _icon_pm('settings', 15, '#9A9AA0')
        p = QPainter(self)
        p.drawPixmap((self.width() - 15) // 2, (self.height() - 15) // 2, pm)
        p.end()


class _PinBtn(QPushButton):
    """타이틀바용 always-on-top 토글 — Lucide 'pin'(압정) 라인 아이콘.
    ON=브랜드 액센트 / OFF=회색(2026-08-30 SPL Alarm 리디자인, 앱 아이콘과 통일)."""
    def __init__(self):
        super().__init__()
    ...
    def paintEvent(self, e):
        super().paintEvent(e)   # hover 배경
        col = T('accent') if self.isChecked() else '#9A9AA0'
        pm = _icon_pm('pin', 15, col)
        p = QPainter(self)
        p.drawPixmap((self.width() - 15) // 2, (self.height() - 15) // 2, pm)
        p.end()
