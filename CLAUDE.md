...
3. **Logs** — for things that need the real app / live audio / hardware (which only the user can run), read the session log instead of screenshots: `~/Library/Logs/WSA2/wsa2_<ts>.log` (newest). Grep `[DIAG]` for the structured heartbeat/snapshots. `_diag(tag, **kv)` (`wayaudo2.py` near the logging setup) writes greppable `[DIAG] tag k=v` lines; a ~10s heartbeat in `_render_frame` logs `view/run/spl/extra/show`. Uncaught exceptions already log full tracebacks (crash handler). Run the app with `WSA2_DEBUG=1` to also stream logs to the console. **Add `_diag(...)` calls at new state transitions** so future sessions can verify behavior from the log. Ask the user to run once + send the log (or run `Open Log Folder` from Help), not screenshots.
