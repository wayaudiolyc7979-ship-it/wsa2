...
- [ ] `[차별화/v1.7+]` **TF 매그+위상 주파수축 줌/팬** (사용자 확정 2026-06-16, v1.7) — 스펙트럼은 불필요(합의), **TF 매그니튜드·위상만**. ⭐**핵심: 매그·위상(+코히런스)이 같은 주파수축이라 줌/팬을 연동**(매그 100~300Hz 확대→위상도 동일 범위). 조작=휠(커서주파수 기준 확대)·드래그(팬)·더블클릭(20~20k 리셋). IR은 제외(시간축, 이미 화살표 이동 있음). ⚠️중간난이도 이유=확대 중 커서·캡쳐곡선·딜레이선·그리드라벨이 다 따라가야 함. 지금 dB(세로)축 휠줌은 이미 있음 → 가로(주파수)축 추가. Smaart도 연동 줌.
...
- [ ] `[참고/이전]` **플로팅 창 풀스크린 (구)** — SPL미터·벡터스코프·라우드니스 팝아웃을 독립 Qt.Window+네이티브 collectionBehavior(FullScreenAuxiliary)로 변경(2026-06-15, 풀스크린 위+최소화 생존 둘 다 OK, 사용자 승인). ⚠️메인 풀스크린일 때 팝업도 크게 뜸. **수정(2026-06-17)**: SPL미터·알람 open이 그냥 show()만 해서 풀스크린 크기로 뜨던 것 → `_force_float_normal()`(벡터스코프 `_show_popout_normal`과 동일 방식: show 직후+140ms 지연 콜백으로 `setWindowState(NoState)`+중앙·정상크기 setGeometry 강제) 적용. 최초 생성 시에만(재오픈 시 사용자 크기 유지). 실측 대기 — macOS 풀스크린 Space가 여전히 덮어쓰면 collectionBehavior 재검토. *(상세: [[project_popout_fullscreen_fix]])*
