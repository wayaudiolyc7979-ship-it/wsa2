...
- **2026-06-17 실앱 검증완료**: SPL 알람 창·always-on-top 자물쇠(깜빡임0)·마우스떠나면 카드만·단축키 치트시트(?)·라우드니스 True Peak·타겟 즉시반영/Δ/LU·벡터스코프↔레이더 원크기·탭 이중네모 제거·TF 분석창 하이라이트 = 전부 OK(사용자 확인). **커서 음이름은 센트(¢) 제거**(음이름만). ⚠️**풀스크린 SPL미터/알람만 실측 미완**(자식창 전환 후 최종 확인 필요). 전부 미빌드.
