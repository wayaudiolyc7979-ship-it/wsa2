...
- [x] **[기능] SPL 임계 알람 — 상단 신호등 바** — ✅구현완료(2026-06-16, 스크린샷 승인 "좋은데"·실앱/하드웨어 미검증·미빌드). **SPL Meter 창 타이틀바 아래 풀폭 신호등 바**(`_SplAlarmBar`, `wayaudo2.py` SplMeterWindow 위) — 마스터 지표 1개를 한계와 비교해 **초록(여유)→노랑(한계 -N dB 접근)→빨강(초과, 0.4s 깜빡임 + "N초째 초과")**. 좌측 3구 신호등 점 + 큰 숫자 + ▼여유/▲초과 dB + 우측 상태 알약(OK/AMBER/OVER). **데이터**: 기존 `push_levels`→`_update_display`(200ms)에서 그대로 읽음(별도 오디오 경로 0). **설정**: ⚙ SPL 설정창에 알람 섹션(표시 on/off·마스터 지표·한계 dB·노랑 여유 dB), `_settings['spl_meter']['alarm']`에 영속. 기본 LAeq/100dBA/-3dB. 오프스크린: 위젯 3상태 렌더 + 창 통합 + 다이얼로그 getter PASS. **버그수정(2026-06-16): 좁은 1열 창에서 글자·알약·점이 고정픽셀이라 겹침** → paintEvent를 너비 반응형으로(좁으면 점1개+값만, 넓어질수록 라벨→/한계→여유→알약 순으로 펼침, 각 요소 들어갈 자리 측정 후만 그림). 210~760px 전구간 겹침 0 확인. **별도 팝업 창 추가(2026-06-16): SPL미터 내장 바 말고 독립 창으로 띄우기** — `SplAlarmWindow`(부모없는 top-level, 다크타이틀바, 풀스크린 위/최소화 생존), `_SplAlarmBar(big=True)`가 창 크기에 맞춰 스케일되는 큰 중앙정렬 레이아웃(3구 신호등 글로우 + 거대 숫자 + /한계 + 여유/초과 + 하단 상태). ⚙ 설정창에 "별도 팝업 창으로 표시" 체크박스, `alarm.popup` 영속. popup 모드면 내장 바 숨김·팝업으로 값 라우팅, 팝업 ✕는 내장바 복귀(설정 popup=False), SPL미터 닫으면 팝업도 정리(silent). 오프스크린: 3상태 팝업 렌더 + 통합 로직(숨김/복귀/재토글/disabled) PASS. 다른 모니터/구석에 띄워 FOH 글랜스용. ⚠️카드 자체 색(빨강)은 dBFS 임계라 SPL주입 시 다 빨강—알람 바와 무관(기존 동작). **다음**=실앱 육안(노랑→빨강 전이·깜빡임)+소리 알람 옵션. 더 큰 그림은 G의 SPL 컴플라이언스 로드맵. *(체크포인트 커밋 60c8321)*
